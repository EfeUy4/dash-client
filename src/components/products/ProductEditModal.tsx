...
// Clothing & accessories categories - same as create modal
...
const ProductEditModal = ({ product, onClose, onSave }: ProductEditModalProps) => {
    ...
    e.preventDefault();
		...
		try {
			...
			const cleanedData = {
				...formData,
				images: formData.images, // Already managed by ImageUpload component
			...
			const { data } = await api.put(`/products/${product.id}`, cleanedData);

			// The response should be the updated product
			onSave(data);
			...
			showErrorToast(error.message || "An error occurred while updating the product");
		} finally {
			setLoading(false);
		}
	};
					...
					<div className="sm:flex sm:items-start">
						<div className="mt-3 text-center sm:mt-0 sm:text-left w-full">
							...
							<form onSubmit={handleSubmit} className="space-y-6">
										...
										<div className="flex items-center justify-between">
											<div>
												...
												<p className="text-xs text-gray-500 mt-1">
													Override automatic stock calculation (useful for quality issues, seasonal items,
													etc.)
												</p>
											</div>
								...
								{/* Physical Properties */}
								<div className="bg-gray-50 p-4 rounded-lg">
									<h4 className="text-md font-medium text-gray-900 mb-4">Physical Properties</h4>
